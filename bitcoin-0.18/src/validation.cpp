                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        if (!bypass_limits && mempoolRejectFee > 0 && nModifiedFees < mempoolRejectFee) {
            logFile(hash.ToString() + " " + strprintf("%d < %d", nModifiedFees, mempoolRejectFee), "min_mempool_fee");
        ...
        if (!bypass_limits && nModifiedFees < ::minRelayTxFee.GetFee(nSize)) {
            logFile(hash.ToString() + " " + strprintf("%d < %d", nModifiedFees, ::minRelayTxFee.GetFee(nSize)), "min_relay_fee");
            return state.Invalid(ValidationInvalidReason::TX_MEMPOOL_POLICY, false, REJECT_INSUFFICIENTFEE, "min relay fee not met", strprintf("%d < %d", nModifiedFees, ::minRelayTxFee.GetFee(nSize)));
        }

        if (nAbsurdFee && nFees > nAbsurdFee)
        {
            logFile(hash.ToString() + " " + strprintf("%d > %d", nFees, nAbsurdFee), "absurdly_high_fee");
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock)
{
    ...
    logFile("CMPCTBLKACCPT - " + pblock->GetHash().ToString());

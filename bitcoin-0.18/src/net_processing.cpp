...
std::vector<std::string> falafel_orphan_txs;
#endif
...
void EraseOrphansFor(NodeId peer)
{
    ...
    while (iter != mapOrphanTransactions.end())
    {
        ...
        if (maybeErase->second.fromPeer == peer)
        {
            ...
            logFile(maybeErase->second.tx->GetHash().ToString(), "orphanTXsNodeRemoval.txt");
#endif
#if (FALAFEL_RECEIVER && LOG_FALAFEL_ORPHAN_TRANSACTIONS)
            auto fOrphanTX = std::find(falafel_orphan_txs.begin(),
                                       falafel_orphan_txs.end(),
                                       maybeErase->second.tx->GetHash().ToString());
            if (fOrphanTX != falafel_orphan_txs.end())
            {
                logFile(maybeErase->second.tx->GetHash().ToString(), "falafel_orphanTXsNodeRemoval.txt");
                falafel_orphan_txs.erase(fOrphanTX);
            }
#endif
#if LOG_CPU_USAGE
            long firstCPUStats = getProcessCPUStats();
            long firstTotalTime = getTotalTime();
#endif
            nErased += EraseOrphanTx(maybeErase->second.tx->GetHash());
#if LOG_CPU_USAGE
            long CPUStats_delta = getProcessCPUStats() - firstCPUStats;
            long TotalTime_delta = getTotalTime() - firstTotalTime;
            logFile(std::to_string(CPUStats_delta) + " " + std::to_string(TotalTime_delta), "cpuusage_erase.txt");
#endif
        }
    }
    ...
    if (nNextSweep <= nNow) {
        ...
        while (iter != mapOrphanTransactions.end())
        {
            ...
            if (maybeErase->second.nTimeExpire <= nNow) {
#if LOG_ORPHAN_TRANSACTIONS
                logFile(maybeErase->second.tx->GetHash().ToString(), "orphanTXsTimeout.txt");
#endif
#if (FALAFEL_RECEIVER && LOG_FALAFEL_ORPHAN_TRANSACTIONS)
                auto fOrphanTX = std::find(falafel_orphan_txs.begin(),
                                           falafel_orphan_txs.end(),
                ...
                if (fOrphanTX != falafel_orphan_txs.end())
                {
                    logFile(maybeErase->second.tx->GetHash().ToString(), "falafel_orphanTXsTimeout.txt");
                    falafel_orphan_txs.erase(fOrphanTX);
                }
#endif
#if LOG_CPU_USAGE
...
#if LOG_ORPHAN_TRANSACTIONS
        logFile(g_orphan_list[randompos]->first.ToString(), "orphanTXsEvicted.txt");
#endif
#if (FALAFEL_RECEIVER && LOG_FALAFEL_ORPHAN_TRANSACTIONS)
        auto fOrphanTX = std::find(falafel_orphan_txs.begin(),
                                   falafel_orphan_txs.end(),
                                   g_orphan_list[randompos]->first.ToString());
        if (fOrphanTX != falafel_orphan_txs.end())
        {
            logFile(g_orphan_list[randompos]->first.ToString(), "falafel_orphanTXsEvicted.txt");
            falafel_orphan_txs.erase(fOrphanTX);
        }
#endif
#if LOG_CPU_USAGE
...
void PeerLogicValidation::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex* pindex, const std::vector<CTransactionRef>& vtxConflicted) {
    ...
    if (vOrphanErase.size()) {
        ...
        for (const uint256& orphanHash : vOrphanErase) {
#if LOG_ORPHAN_TRANSACTIONS
            logFile(orphanHash.ToString(), "orphanTXsResolvedByBlock.txt");
#endif
#if (FALAFEL_RECEIVER && LOG_FALAFEL_ORPHAN_TRANSACTIONS)
            auto fOrphanTX = std::find(falafel_orphan_txs.begin(),
                                       falafel_orphan_txs.end(),
            ...
            if (fOrphanTX != falafel_orphan_txs.end())
            {
                logFile(orphanHash.ToString(), "falafel_orphanTXsReslovedByBlock.txt");
                falafel_orphan_txs.erase(fOrphanTX);
            }
#endif
#if LOG_CPU_USAGE
...
#if LOG_TX_RELAY
        if (pnode->fRelayTxes)
            logFile(inv, pnode->GetAddrName(), 0);
#endif
...
void static ProcessGetData(CNode* pfrom, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc) LOCKS_EXCLUDED(cs_main)
{
...
#if FALAFEL_SENDER
    bool from_falafel_node = pfrom->GetAddrName().find("128.197.128.") != std::string::npos;
#endif
    ...
    const CNetMsgMaker msgMaker(pfrom->GetSendVersion());
    {
        ...
        while (it != pfrom->vRecvGetData.end() && (it->type == MSG_TX || it->type == MSG_WITNESS_TX)) {
            ...
            {
#if FALAFEL_SENDER
                if(from_falafel_node)
                {
                    logFile((*it).hash.ToString(), "buffer_processgetdata.txt");
                    //logFile(pfrom->vRecvGetData.begin(), pfrom->vRecvGetData.end(), pfrom->GetAddrName());
                }
#endif
                break;
            }
...
#if FALAFEL_SENDER
                if(from_falafel_node)
                    logFile(inv.hash.ToString(), "falafel_tx_sent_map_relay.txt", pfrom->GetAddrName());
#endif
                push = true;
...
#if FALAFEL_SENDER
                    if(from_falafel_node)
                        logFile(inv.hash.ToString(), "falafel_tx_sent_mempool.txt", pfrom->GetAddrName());
#endif
                    push = true;
                }
            }
...
#if FALAFEL_SENDER
                if(from_falafel_node)
                    logFile(inv.hash.ToString(), "falafel_not_found.txt", pfrom->GetAddrName());
#endif
...
void static ProcessOrphanTx(CConnman* connman, std::set<uint256>& orphan_work_set, std::list<CTransactionRef>& removed_txn) EXCLUSIVE_LOCKS_REQUIRED(cs_main, g_cs_orphans)
{
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(mempool, orphan_state, porphanTx, &fMissingInputs2, &removed_txn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
            ...
            if (fOrphanTX != falafel_orphan_txs.end())
            {
                logFile(orphanHash.ToString(), "falafel_orphanTXsResolved.txt");
                falafel_orphan_txs.erase(fOrphanTX);
            }
#endif
#if LOG_ORPHAN_TRANSACTIONS
            logFile(orphanHash.ToString(), "orphanTXsResolved.txt");
#endif
#if LOG_CPU_USAGE
            long firstCPUStats = getProcessCPUStats();
            long firstTotalTime = getTotalTime();
            ...
            if (fOrphanTX != falafel_orphan_txs.end())
            {
            logFile(orphanHash.ToString(), "falafel_orphanTXsRemoved.txt");
            falafel_orphan_txs.erase(fOrphanTX);
            }
#endif
#if LOG_ORPHAN_TRANSACTIONS
            logFile(orphanHash.ToString(), "orphanTXsRemoved.txt");
#endif
#if LOG_CPU_USAGE
            long firstCPUStats = getProcessCPUStats();
            long firstTotalTime = getTotalTime();
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
            ...
            else
            {
                ...
                if (fBlocksOnly) {
                    ...
                    if(falafelInv)
                    {
                        ...
                        logFile(inv, pfrom->GetAddrName(), "falafelMissingTXReqSent.txt");
                    }
#endif
                }
            }
    ...
    if (strCommand == NetMsgType::TX) {
        ...
        if (!AlreadyHave(inv) &&
...
#if (LOG_TRANSACTIONS || LOG_FALAFEL_TRANSACTIONS)
            std::string duration = std::to_string(
                    std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count() - start);
#if LOG_TRANSACTIONS
            logFile(tx, pfrom->GetAddrName(), duration);
#endif
#endif
            ...
            {
                logFile(inv.hash.ToString(), "missingInvResTrack.txt", pfrom->GetAddrName());
#if LOG_FALAFEL_TRANSACTIONS
                logFile(tx, pfrom->GetAddrName(), duration, "falafel_txs.txt");
#endif
        ...
        else if (fMissingInputs)
        {
            ...
            if (!fRejectedParents) {
...
#if (FALAFEL_RECEIVER && LOG_FALAFEL_ORPHAN_TRANSACTIONS)
                bool isFalafelTX = std::find(falafel_missing_invs.begin(),
                                             falafel_missing_invs.end(),
                                             ptx->GetHash().ToString()) != falafel_missing_invs.end();
#endif
...
#if (FALAFEL_RECEIVER && LOG_FALAFEL_ORPHAN_TRANSACTIONS)
                        if (isFalafelTX)
                            logFile(_inv.hash.ToString(), "falafel_parents/" + tx.GetHash().ToString());
#endif
#if LOG_ORPHAN_TRANSACTIONS
                        //fix this
                        logFile(_inv.hash.ToString(), "parents/" + tx.GetHash().ToString());
#endif
                ...
                long CPUStats_delta = getProcessCPUStats() - firstCPUStats;
                long TotalTime_delta = getTotalTime() - firstTotalTime;
                logFile(std::to_string(CPUStats_delta) + " " + std::to_string(TotalTime_delta), "cpuusage_add.txt");
#endif
#if (FALAFEL_RECEIVER && LOG_FALAFEL_ORPHAN_TRANSACTIONS)
                if (isFalafelTX)
                {
                    logFile(ptx->GetHash().ToString(), "falafel_orphanTXsAdded.txt", pfrom->GetAddrName());
                    falafel_orphan_txs.push_back(ptx->GetHash().ToString());
                }
#endif
#if LOG_ORPHAN_TRANSACTIONS
                logFile(ptx->GetHash().ToString(), "orphanTXsAdded.txt", pfrom->GetAddrName());
#endif
...
#if (LOG_ORPHAN_TRANSACTIONS || LOG_FALAFEL_ORPHAN_TRANSACTIONS)
        logFile(std::to_string(lRemovedTxn.size()), "orphan_AddToCompactExtraTransactions.txt");
#endif
    ...
    if (strCommand == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (pindex->nHeight <= ::ChainActive().Height() + 2) {
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                logFile("cmpctblock", "blockType.txt", pindex->GetBlockHash().ToString());
    ...
    if (strCommand == NetMsgType::BLOCKTXN)
    {
        ...
        bool fBlockRead = false;
        {
            ...
            if (status == READ_STATUS_INVALID) {
                ...
                logFile("TXNFILLFAIL - blocktxn message recived but unsuccessful, now preforming getdata RT\'s: " + pblock->GetHash().ToString());
            } else {
                ...
                logFile("TXNFILLSUCCESS - blocktxn message recived and successfully filled missing TXN: " + pblock->GetHash().ToString());
            }
    ...
    if (strCommand == NetMsgType::BLOCK)
    {
        ...
        logFile("normalblock", "blockType.txt", pblock->GetHash().ToString());
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
...
#if (LOG_ORPHAN_TRANSACTIONS || LOG_FALAFEL_ORPHAN_TRANSACTIONS)
        logFile(std::to_string(removed_txn.size()), "orphan_AddToCompactExtraTransactions.txt");
#endif
    ...
    if (pfrom->fPauseSend)
    {
        ...
        bool from_falafel_node = pfrom->GetAddrName().find("128.197.128.") != std::string::npos;
        if(from_falafel_node)
            logFile("Send buffer full", "buffer_processmessages.txt");
#endif
        return false;
    }
            ...
            if (!AlreadyHave(inv)) {
                ...
                if (last_request_time <= nNow - GETDATA_TX_INTERVAL) {
                    ...
                    if(std::find(falafel_missing_invs.begin(),
                        ...
                        logFile(inv.hash.ToString(), "missingInvReqTrack.txt", pto->GetAddrName());
#endif

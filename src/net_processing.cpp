...
extern std::unordered_map<std::string, std::tuple<int64_t, bool, bool>> connect_times;
...
bool ProcessMessage(CNode *pfrom, std::string strCommand, CDataStream &vRecv, int64_t nStopwatchTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION)
    {
        ...
        /*
        if(isNodeInWhiteListedSubnets(pfrom->addrName))
        {
            logFile(pfrom->GetLogName(), "addr.txt");
            connect_times[pfrom->addrName] = {GetTime(), pfrom->GrapheneCapable(), false};
            if(pfrom->GrapheneCapable()) pfrom->nServices &= ~NODE_GRAPHENE;
            logFile("SRVCDSBL -- Graphene service disabled for peer " + pfrom->GetLogName());
        }
        */
...
{
    /*
    if(isNodeInWhiteListedSubnets(pto->addrName) && connect_times.find(pto->addrName) != connect_times.end() && !std::get<2>(connect_times[pto->addrName]) && GetTime() - std::get<0>(connect_times[pto->addrName]) >= GRAPHENE_SERVICE_RESTORE_TIMEOUT)
    {
        if(std::get<1>(connect_times[pto->addrName]))
        {
            logFile("SRVCRST -- Graphene service restored for peer " + pto->GetLogName());
            pto->nServices |= NODE_GRAPHENE;
        }
        std::get<2>(connect_times[pto->addrName]) = true;
    }
    */
    const Consensus::Params &consensusParams = Params().GetConsensus();
    {

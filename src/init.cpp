...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if(!initLogger())
    {
        ...
        LogPrintf(">>  Logger system initialized");

#if LOG_NEIGHBOR_ADDRESSES

    if(initAddrLogger())
    {
        LogPrintf(">> Neighbor address logger initialized");
        threadGroup.create_thread(boost::bind(AddrLoggerThread, &(*g_connman)));
        LogPrintf(">> Neighbor address logger thread created");
    }
    else
    {
    ...
    {
        LogPrintf(">> Process CPU usage logger initialized");
        threadGroup.create_thread(CPUUsageLoggerThread);
        LogPrintf(">> Process CPU usage logger thread created");
    }
    else
    {
    ...
    {
        LogPrintf(">> Mempool Sync trigger timer initialized");
        threadGroup.create_thread(std::bind(mempoolSyncTriggerTimerThread, &(*g_connman)));
        LogPrintf(">> Mempool Sync trigger timer thread created");
    }
    else
    {
        LogPrintf(">> ERROR: initTimerThread failed");
    }
